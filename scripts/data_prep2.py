...
def prepare_one_year(
    *,
    ...
    """Prepare a single DE micro-data year and persist the cleaned parquet."""
    raw_dir = raw_dir or euromod_raw_root()
    out_dir = out_dir or (data_root() / "processed" / "de" / str(year))
    ...
    try:
        clean_df = clean_harmonize_de(data_df)
    finally:
    ...
    out_file = out_dir / f"de_{year}_clean.parquet"
...
def _cli_prepare() -> None:
    parser = argparse.ArgumentParser(description="Prepare DE micro-data for a given year.")
    parser.add_argument("--year", type=int, required=True)
    parser.add_argument("--raw-file", required=True, help="e.g., DE_2015_a1.txt")
