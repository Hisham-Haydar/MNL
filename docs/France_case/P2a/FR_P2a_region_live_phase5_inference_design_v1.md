# FR P2a Region-Live — Phase-5 Inference Design — v1

**Mission:** JMP-M05 Stage B — household-clustered inference design
**Mode:** design only; no implementation, no computation, no commit
**Date:** 2026-07-31
**Target repository path:** `MNL/docs/France_case/P2a/FR_P2a_region_live_phase5_inference_design_v1.md`
**Commit status:** UNCOMMITTED pending Stage-C independent methods review and Stage-E manager acceptance
...
The verdict is not `READY FOR MANAGER REVIEW` for two reasons, both structural rather than derivational:

1. **[C-4] mandates independent review of the central object.** The 35-dimensional conditional covariance is a working hypothesis under [C-4] and cannot be closed by the design author. This memo recommends it, states the exact condition under which it is asymptotically valid, and pre-registers its falsification criterion; ratification is Stage C's to give (§22, D-2).
2. **One `UNKNOWN` conditions the score-artifact recommendation.** Whether household-level derived arrays from EU-SILC/EUROMOD input data may be committed to version control depends on the licence terms and the repository's disclosure status. Neither is a repository fact the Stage-A audit established, so it is `UNKNOWN` here (§22, D-3). A pre-specified fallback is defined so that this cannot block execution.
...
Terminology, fixed here under [C-3]: the estimator is described as **household-clustered and misspecification-robust**, and the algebraic equivalence to the OPG sandwich is stated wherever the estimator is named. The sandwich is misspecification-robust, **not dependence-robust**: with one term per cluster, clustering removes no dependence, because there is none across contributions to remove [audit §13]. No claim is made or implied that clustering will remain degenerate for couples or for pooled years; those cases depend on the primitive contribution and repeated-unit structure actually implemented, which is not this application [C-3].
...
### 10.3 What `K` counts, and why 35

`K = 35`. The justification is not that 35 is the dimension of the reported table, but that **35 is the number of first-order conditions the estimation actually imposed on the score sums.** At the accepted estimate the scores satisfy `Σ_g s_{g,j} ≈ 0` for the 35 interior coordinates (max `1.0993e-4` [P4D]), while for the two active coordinates `Σ_g s_{g,j}` equals `0.8445544161794221` and `1.4682021491125388` — strictly non-zero and O(1) [P4D], [audit §8]. The degrees-of-freedom loss in a sandwich meat is the number of exactly-satisfied linear restrictions on the scores, which is 35, not 37.
...
| Rejected | Reason |
| --- | --- |
| CR0 | Defensible asymptotically but conventionally under-corrected; a labour-economics referee expects a finite-sample factor, and its absence would read as an omission rather than a choice. |
| Cluster-only `G/(G−1)` | At `G = 1,555` this is +0.03 % — numerically indistinguishable from CR0 while carrying the rhetorical weight of a correction. It buys nothing and obscures the parameter-count adjustment that actually has content here. |
| Two-factor with `K = 37` | Coherent, and within 0.07 % of the baseline; rejected because it counts two coordinates whose first-order conditions are *not* satisfied, so the df interpretation does not hold for them. |
...
If the implementation finds that `N ≠ G` — that the number of additive likelihood terms differs from the number of clusters — or that any weighting enters the objective, the telescoping fails, `K`'s interpretation becomes ambiguous, and the design falls back to the cluster-only factor `G/(G−1)` with the ambiguity documented, rather than choosing a factor whose inputs it cannot define [plan §6.3]. On the verified facts this cannot trigger without a change to the accepted model. Separately: **if the covariance object is ever changed from the conditional-35 to the unrestricted-37 (§11), `K` must move to 37 in the same edit.** `K` is tied to the covariance object's dimension, and the two decisions are not independent.
...
- Objects 2 and 3 coincide **if and only if** `H_I𝒜 = 0`. Since `H_I𝒜 H_𝒜𝒜⁻¹ H_𝒜I ⪰ 0` for positive-definite `Hs`, we have `𝒮 ⪯ H_II` in the Loewner order and therefore `H_II⁻¹ ⪯ 𝒮⁻¹ = [Hs⁻¹]_II`. **The conditional covariance is weakly smaller than the marginal one.** Conditioning on the active set does not produce conservative standard errors; it produces *conditional* standard errors. This must be said in the paper, and §20 requires it.
...
**The two bound coordinates are reported as follows, and no other way.** Estimate `1.0`; bound value `1.0`; direction `upper`; the negLL gradient component; multiplier; and the status flag string **`active-bound`**. **No standard error, no z-statistic, no p-value, no confidence interval** [C-4], [plan §7.2(2)]. The status flag text is fixed here so that implementation cannot improvise: the SE, z and p fields take the literal string `NA`.
...
- **T-22 (gating).** Both multipliers must exceed `100 ×` the interior maximum `|∇negLL|`. Observed ratios 7,682.9 and 13,356.3 [arith] clear this by two orders of magnitude. If a future recomputation of the accepted state fails this, the strict-activity argument collapses, the equality-restriction treatment is falsified, and the coordinate must instead be treated as interior-but-near-boundary with the limitation reported [plan §7.3].
...
- **W-4 (warning, mandatory escalation).** For each interior parameter, the two-sided 95 % Wald interval must lie strictly inside that parameter's own spec bounds. Two coordinates are the plausible candidates to trip it: `sigma`, accepted `0.413492` with lower bound `0.10` (distance `0.313`), and `theta_c_singles`, accepted `0.093459` with upper bound `0.95` (distance `0.857`) [map]. Whether either trips is `UNKNOWN` until standard errors exist. If one does, it is flagged in the table and escalated to the manager; the boundary machinery of §11.3 is **not** extended to it in Phase 5, because it is interior and its first-order condition holds.

### 11.6 Economic reading, carried as a caveat not resolved here

These are the age-squared terms in the singles leisure block — the curvature of the leisure-preference profile in age. Their sitting at a corner means the estimated preference block absorbs age curvature at the constrained extreme. Because age-in-leisure routes to the preference channel under the project's normative bookkeeping, and because the headline decomposition separates opportunity from preferences [DL D-011], the concern must be stated plainly: **a preference block pinned at a corner cannot flexibly absorb variation, which bears on the interpretation of any absorption result in JMP-M07** [plan §7.2(4)]. Whether the bound is tight because of the covariate's scaling (`age_norm2`) or because of substantive curvature is `UNKNOWN` and is not investigated here. This is a caveat for the manuscript and a handoff item for M07/M08 — not evidence about the specification, not a defect to be resolved in Phase 5, and not grounds for respecification within this mission.
...
**Baseline: all ten pins appear in the parameter table with their pinned value, a category label, and the literal string `NA` in the SE, z and p fields, under the status flag `pinned`, with a table footnote.**
...
The statistic, for a selection matrix `R` of shape `q × 35` and `r = 0`:

```
W = (R θ̂_ℐ)ᵀ (R V_RR-relevant Rᵀ)⁻¹ (R θ̂_ℐ)     computed by Cholesky solve, never explicit inverse
W ~ χ²(q)  asymptotically under H0
```

| Null | `q` | Restriction | Status |
| --- | --- | --- | --- |
| **H0-A** | **10** | all ten access coordinates zero | **certified omnibus** — the single confirmatory test [charter, requirement G] |
| H0-B | 7 | the seven NUTS-1 dummies jointly zero | pre-registered **primary sub-test**; maps most directly onto the "common opportunity environment" counterfactual [plan §9.2(1)] |
| H0-C | 2 | `drgur`, `drgmd` jointly zero | pre-registered secondary |
| H0-G | 1 | `beta_E_gsur = 0` | pre-registered secondary, **stated separately**; `gsur` belongs to neither H0-B nor H0-C [F-3] |

Each is computed in both model-based and robust forms; the robust form carries the reported verdict. Critical values `χ²(10)₀.₉₅ = 18.3070`, `χ²(7)₀.₉₅ = 14.0671`, `χ²(2)₀.₉₅ = 5.9915`, `χ²(1)₀.₉₅ = 3.8415` [arith].
...
| **T-4** | Free-gradient reproduction against recorded `gradient_free`, max abs dev ≤ `1e-12` (Phase 4 achieved `8.88e-16` against Phase 3) | gating |
...
| **T-7** | Meat validity: `max\|M − Mᵀ\| ≤ 1e-12 · max\|M\|` before symmetrisation; `min_eig(M) ≥ −1e-8 · max_eig(M)`; rank recorded | gating |
...
| **T-9** | Covariance validity: `V_model` and `V_robust` symmetric to `1e-12` relative; `V_model` positive definite; `V_robust` PSD with `min_eig ≥ −1e-8 · max_eig`; all 35 diagonal entries of both strictly positive and finite | gating |
...
| **T-22** | Active-set strictness: `μ_sm` and `μ_sf` each ≥ `100 ×` interior max `\|∇negLL\|` | gating |
...
| **W-4** | Near-boundary containment: two-sided 95 % Wald interval of every interior parameter lies strictly inside its own spec bounds; violations flagged and escalated to the manager | warning, mandatory escalation |
...
| --- | --- | --- |
| Meat / covariance PSD | `min_eig ≥ −1e-8 × max_eig` | Pre-registered at [plan §11.1 T-7]; one order looser than the rank convention, appropriate for an accumulated sum of 1,555 outer products |
...
| Active-set strictness (T-22) | `≥ 100 ×` interior max gradient | Observed margin is 7,683 and 13,356 [arith] — two orders of magnitude of headroom |
...
**Rejected:** CSV as the sole or canonical artifact (hash fragility, demonstrated twice in this repository); hashed binary held outside version control with only summaries committed (loses direct reproducibility for a 0.44 MiB object, with no offsetting benefit **unless** the disclosure question of §22 D-3 resolves against committing, which is exactly the pre-specified fallback).

**Pre-registered falsification criterion.** If the licence or disclosure determination (§22 D-3) returns that household-level derived arrays may not be committed, the baseline switches to the pre-specified fallback — hashed binary stored outside version control, with the SHA-256, the shape, the ordered column names, and the full summary table committed — with no further design cycle required.
...
| `phase5_parameter_table.csv` | 47 rows: name, block, status, estimate, model SE, robust SE, ratio, z, p, flag | yes |
| `phase5_regional_tests.csv` | null id, `q`, `W_model`, `W_robust`, p, critical value | yes |
...
The parameter table has exactly these columns: `name`, `block`, `estimate`, `se_model`, `se_robust`, `ratio_robust_model`, `z`, `p`, `status`. `status ∈ {interior, active-bound, pinned}`. For `active-bound` and `pinned` rows, `se_model`, `se_robust`, `ratio`, `z`, `p` all take the literal string `NA` (§11.3, §12.2). Reported precision is local at the accepted estimate and carries no identification or recovery claim [plan S-9].
...
**Downstream declaration, pre-registered** [plan S-10]. Delta-method propagation into M08/M09 will consume the 35 × 35 interior covariance. The two active-bound coordinates enter downstream functionals as fixed, and the ten pins likewise; both **understate** decomposition uncertainty, and the direction of that understatement is known and must be stated. Pre-registered trigger: **if any pinned or active-bound coordinate loads materially on the welfare or opportunity index, a resampling-based uncertainty treatment becomes a requirement of a later mission rather than an optional robustness item** — X-005 currently defers the decomposition bootstrap [DL X-005], and this trigger is the condition under which that deferral must be revisited.
...
- "The finite-sample correction is `c = G/(G−K) = 1555/1520 = 1.0230`, inflating standard errors by 1.14 %."
...
- ❌ Presenting the conditional standard errors as unconditional, or as conservative. They are conditional on the active set and are weakly **smaller** than the marginal object (§11.2).
...
| --- | --- | --- | --- |
| **D-1** | Finite-sample correction | Two-factor with `N = G = 1,555`, `K = 35`, telescoping to `c = G/(G−K) = 1.0230263157894737` (§10) | recommended; ready to freeze |
| **D-2** | Active-bound treatment | Conditional 35 × 35 sandwich; the two bound coordinates reported with `NA` and flag `active-bound`; no symmetric Wald inference (§11) | recommended; **[C-4] requires independent review before freezing** |
| **D-3** | Score artifact | `.npy` authoritative + committed summaries + non-authoritative `%.17g` CSV rendering (§17.1) | recommended; **conditional on the disclosure determination below** |
| **D-4** | Fixed-pin representation | Literal `NA` in SE/z/p; two structural-inapplicability categories; **no normalisation category**; mandatory footnote (§12) | recommended; ready to freeze |
| **D-5** | Regional covariance and joint-test protocol | H0-A (`q = 10`) certified omnibus; H0-B (7), H0-C (2), H0-G (1) pre-registered secondary; `gsur` stated separately; χ² reference; joint test carries the claim (§13) | recommended; ready to freeze |
| **D-6** | Numerical gates and tolerances | The T-/W- register of §14–§16, extending the accepted [plan §11] naming | recommended; ready to freeze |
| **D-7** | Canonical row order | `idhh`-ascending, stable argsort (§6.3) | recommended; ratification requested because it determines artifact hashes |
...
1. **[C-4] independent review of D-2.** The conditional-35 object is a working hypothesis under the accepted corrections and requires the Stage-C reviewer to rule on: the exact active-bound direction and KKT consistency (§11.1, verified); whether conditioning on the active set is the intended estimand (§11.3, argued); the correct conditional bread and meat (§9.1); the limits of treating the active set as fixed (§11.3, §20.2); and whether any alternative boundary-aware inference is required for paper claims (§11.3, deferred with a trigger). This is the priority review target.
2. **`UNKNOWN` — disclosure status of household-level derived arrays.** The 1,555-row score matrix is derived from EU-SILC/EUROMOD input microdata. Whether it may be committed to version control, and whether it may appear in any future public replication package, depends on the Eurostat/EUROMOD licence terms and on the repository's disclosure status. Neither is a repository fact the Stage-A audit established, so both are `UNKNOWN` here. The manager should record a one-line determination. **A pre-specified fallback exists (§17.1), so this cannot block execution.** A minimum recommendation regardless of outcome: the manifest carries a `disclosure_class` field, and any future public release excludes household-level rows unless the determination explicitly permits them.
3. **Ratification of D-7**, since the audit explicitly required the memo to fix one of two candidate row orders [audit §14] and the choice determines every artifact hash.
...
Return this memo, unmodified and **uncommitted**, together with a cover note stating the verdict, the three charter-mandated baselines (D-1 finite-sample correction; D-2 active-bound treatment; D-3 score artifact), and the open items, to the **Goal 1 Manager chat only** — not to the deputy programme director.

On the Goal 1 Manager's receipt, the expected sequence is: Stage C, independent methods review, delivered as `FR_P2a_region_live_phase5_inference_methods_review_v1.md` by a separate reviewer, with the [C-4] item (D-2) designated the priority review target; then Stage D, at most two narrow remediation cycles, none used to date; then Stage E, the Goal 1 decision packet `JMP_M05_goal_manager_acceptance_v1.md` and the return to the deputy programme director with a recommendation on whether to launch the Phase-5 implementation mission.

Do not commit this file before independent review and manager acceptance. Do not implement Phase 5. Do not run inference.
