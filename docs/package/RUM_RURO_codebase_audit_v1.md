# RUM/RURO Codebase Audit — v1

**Date:** 2026-05-27  
**Status:** Documentation only — no code was modified  
**Produced by:** Automated audit against codebase state as of 2026-05-27  
**Companion documents:**  
- `RUM_RURO_package_refactor_plan_v1.md` — architecture and migration plan  
- `PROMPT_RUM_RURO_refactor_plan_2026-05-26.md` — verbatim prompt that produced this audit  
...
2. **Stage 5 — data assembly**:
   - Set `prior = 1.0` for all rows (deterministic grid → uniform importance weight;
     see Section F for why this is sufficient)
   - No other changes to `enh_RURO_prep_mnl_basic.py` are anticipated

3. **Stage 6 — estimation**:
   - No engine changes needed (see Section F, Subsection F.4)
   - Spec YAML needs a `model_type: rum` annotation (informational only)
   - Equivalence test: compare RUM LL against independently computed LL
...
**How it works in RURO:**

In the current engine (`estimation_engine.py:363`):
```
V = u + log_h + log_w + log_market − np.log(data.prior)
```

The term `− np.log(data.prior)` is the importance-sampling correction.  
`data.prior` = proposal density π(h, w) evaluated at each draw.  
In RURO, draws are random (Uniform), so π ≠ 1 and the correction is non-trivial.

**Why RUM does not need this correction:**

In RUM the alternatives are a deterministic, uniformly-weighted grid.  Every alternative
has weight 1 / n_alts.  No importance sampling is performed.  The correction term should
equal zero.

**Current state:**

There is **no `model_type` flag** in `estimation_engine.py` to suppress the correction.
The terms `g1`, `g2`, `q_proposal` do not appear in the engine; correction is implicit
via `data.prior`.

**RUM compatibility solution — no engine change needed:**

Setting `prior = 1.0` for all rows in the RUM data-assembly stage (Stage 5) makes
`− np.log(1.0) = 0`, so the correction vanishes automatically.  The engine then computes:

```
V = u + log_h + log_w + log_market − 0  =  u + log_h + log_w + log_market
```

For a standard RUM (no wage draws, no occupation draws), `log_h` and `log_w` should also
be zero (or omitted from the utility spec), leaving `V = u + log_market`.

**Implementation requirement:** The new RUM Stage 3 module must write `prior = 1.0` for
all alternatives.  No modification to `estimation_engine.py` is required.  
A `model_type: rum` annotation in the spec YAML is recommended for documentation and
future-proofing, but need not be read by the engine itself.
