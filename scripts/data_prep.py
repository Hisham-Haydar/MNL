...
data_file = os.path.join(data_dir, "DE_2015_a1.txt")
...
print(
    f"the columns and info After createion of aggregatres{final_df_sim1.info('memory_usage')}" # type: ignore
)
...
# # =================================================
# # Section 1: Data Loading and Sample Restriction
# # =================================================
# # Assume final_df_sim1 is already in memory.
# df = final_df_sim1.copy()

# # Restrict to individuals aged 16+ and eligible (household head or partner)

# df = df[(df["hh_IsHead"] == 1) | (df["tu_hh_de_IsPartner"] == 1)]

# # Create work indicator: work = 1 if monthly income (yem) > 500, else 0.
# df["work"] = (df["yem"] > 500).astype(int)
...
# Drop rows where dgn values match specific conditions
df_couples = df_couples[
    ~df_couples["idhh"].isin(
        df_couples[
