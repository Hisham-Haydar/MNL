...
# RURO Model-Specification Contract v3: continuous-RURO Occupation

**File to save as:** `RURO_model_spec_contract_v3_ruro_occ.md`
**Replaces (for the next estimation step):** §6 and §10 of `RURO_model_spec_contract_v2_continuous_enhanced.md`. Other sections of v2 remain valid where not contradicted here.
...
1. Locks the third opportunity layer of the enhanced continuous RURO model as **occupation opportunity** based on `loc` / `loc4`.
...
- **Working**: `(h, w, occ)`, where `h ∈ [h_min, h_max]` and `w ∈ [w_min, w_max]` are continuous draws (the R reference's uniform proposal is acceptable; an empirical proposal may be substituted with documentation in `mnlmeta.json`), and `occ ∈ {1, …, K}` is drawn from a documented occupation proposal `q_Occ`.
...
9. **New YAML spec `estimation_spec_ruro_occ_M0.yaml`** in `scripts/enhanced/`, encoding §24.

10. **New diagnostics script `scripts/diagnostics/run_recovery_test_ruro_occ.py`** (stub allowed at audit time; full implementation in the patch plan).
...
11. **Recovery harness.** Verify or scaffold `scripts/diagnostics/run_recovery_test_ruro_occ.py`. Do not run; verify entry point.
...
**Save this contract as `RURO_model_spec_contract_v3_ruro_occ.md`** in the project root or `docs/`. Use it as input to the next Claude Code task: produce `RURO_code_contract_audit_v3.md` against the current `scripts/enhanced/` codebase. The patch plan is a separate step.