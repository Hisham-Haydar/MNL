# JMP Welfare Specification Memo v4
...
`JMP_welfare_spec_v2.md`, and `JMP_welfare_spec_v3.md`. The single contract a later
...
**Real validation gates (not interpretive identities).** Two checks the scaffold
...
measure — the order-independence/exhaustiveness gate. (b) The proposal-correction
simulation-consistency check: welfare quantities stable as the draw count grows.
*Not a gate:* the $W^1$/$W^5$ dual is a conceptual mapping of the access/ability
cut onto the menu, **not** a numerical identity — $W^1$ and $W^5$ are different
money metrics with different references, so their inequality gap is not the same
quantity as any single measure's Shapley access/ability components, and there is
no theorem equating them. Report the dual spread alongside the decomposition as
...
pooled $\hat\theta$, with the active measure set read from configuration and the
following written in as required, pre-computation validation gates: the
...
exhaustiveness gate (components sum to $I(\Omega^k)$); and the proposal-correction
simulation-consistency check. The $W^1$/$W^5$ dual is reported as corroborating
interpretation, not as a reconciliation gate.

**Save this as `JMP_welfare_spec_v4.md`** (design memo; methodology docs folder,
superseding `JMP_welfare_spec_v1.md`, `JMP_welfare_spec_v2.md`, and
`JMP_welfare_spec_v3.md`).
