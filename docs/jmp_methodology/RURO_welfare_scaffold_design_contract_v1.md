# RURO Welfare-Scaffold Design Contract v1
...
`JMP_welfare_spec_v4.md` (and the two v2 welfare memos it supersedes the object
layer of) into a written code contract.
...
- **Gender-split robustness.** Baseline welfare uses **shared** `beta_E`,
  `beta_h_pt2` (v4 §3c). Gender-differentiated offers are a planned robustness
  only; `beta_h_pt2` is excluded from any identified gendered path
  (`RURO_gsplit_nonid_structure_v1.md`: independent mislocation, not
  reparameterisable). A future `beta_E` contrast swap is a config change, deferred.
- **Stochastic dominance** of choice sets (v4 §5): deferred until the family
  numbers exist.
...
- The draw count and scheme (B-pool D1+W1; 901 couples, 101 singles) MUST be
  recorded in the output provenance, and the proposal-correction
  simulation-consistency gate (§6) MUST pass before any `V_i`-derived distribution
  is trusted.
...
- Singles male, singles female, and couples are processed under type-conditional
  ...
  opportunity/welfare headline — §7, and v4 §3a).
...
- `J` (universal job set for `W^6`): the pooled support of offered job types,
  declared in config (v4 §1.2). Not a hardcoded France/2016 enumeration.
...
welfare:
  ...
  core:
    ...
    simulation_consistency:
      enabled: true                                  # §6 gate
      draw_multipliers: [1, 2, 4]                    # welfare must be stable as draws grow
  ...
  unit:
    ...
    pool_opportunity_share: false                    # couples/singles NEVER pre-pooled (v4 §3a)

  fixed_params:                                       # mirrors estimation fixed_params; held in welfare too
    theta_l_m: -0.8
    beta_ll: 0.0
  pinned_zero_uncertainty: ["theta_l_m", "beta_ll"]   # bootstrap holds these fixed (v4 §3a)

  blocks:                                             # channel membership read from config, not hardcoded
    preference: [...]                                 # v (tastes); per v4 §2 table
  ...
  decomposition_readiness:                            # interfaces only; NOT implemented here
    preference_equalisation_pinned_switch: "held"     # "held" | "swapped" (v4 §3a) — see §7
...
equals the across-measure spread, which is unknown until computed (v4 §6). The
scaffold is therefore built and validated in this order; **no** "sensitivity to the
welfare measure is the result" claim is made before the spread is observed.
...
with the `-log π(j)` correction and passing the simulation-consistency gate (§6);
...
access channel behind the measure, the opposite of a first stress-test — v4 §6.)
...
1. **Proposal-correction simulation-consistency.** Welfare quantities (`V_i` and
   each `Omega_i^k`'s inequality) must be **stable as the draw count grows**
   (`core.simulation_consistency.draw_multipliers`). A drift beyond a declared
   tolerance as draws increase indicates the `-log π(j)` correction or the
   inclusive-value sum is mis-specified; the gate fails and no distribution from
   that configuration is reported. This is the v4 §1.1 / §6 mandatory check.
...
- **Pinned-preference switch exposed as a config flag.** `theta_l_m` and `beta_ll`
  ...
  (v4 §3a). This choice sizes the couples preference component and therefore the
  couples opportunity share, so it MUST be a config flag
...
- **Couples and singles opportunity shares kept separable, never pre-pooled.** The
  ...
  (v4 §3a). The outputs MUST keep singles and couples opportunity shares separable;
  the scaffold MUST NOT emit a single pooled opportunity-share object
  (`unit.pool_opportunity_share: false`).
...
- **Bootstrap cost scales with menu size.** Each replicate recomputes every active
  ...
  **tighter** than preference CIs (the opportunity blocks are an order of magnitude
  more precisely estimated than the leisure blocks — v4 §3b); the scaffold states
  this expectation, it does not assert it as a result.
...
- **The `W^1`/`W^5` dual is corroborating interpretation, not a numerical
  ...
  reconciliation/identity gate (v4 §6).
...
**Save this as `docs/jmp_methodology/RURO_welfare_scaffold_design_contract_v1.md`.**
