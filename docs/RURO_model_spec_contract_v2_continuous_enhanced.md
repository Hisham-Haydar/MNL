...
# RURO Model-Specification Contract v2: Continuous Enhanced

**File to save as:** `RURO_model_spec_contract_v2_continuous_enhanced.md`
...
This contract supersedes v1's choice of the bundled job-choice model as the target specification for the next estimation step. It defines the target as a **enhanced continuous RURO model** built on the continuous-draws enhanced pipeline (`scripts/enhanced/enh_RURO_draws.py` → `enh_RURO_euromod.py` → `enh_RURO_prep_mnl_basic.py` → `enh_RURO_estimate_FR.py`), augmented with one additive occupation-opportunity layer.
...
- **A clean continuous-RURO reference design exists in the same repository** (the continuous enhanced branch), is closer to the literature (Aaberge–Colombino, Dagsvik–Jia, Jacquet–Jia–Thoresen), and has a transparent additive structure that maps directly onto the welfare-decomposition counterfactuals in the JMP.

The job-choice branch is retained as a **robustness extension**, not the target.

---

## 3. Target enhanced continuous RURO model
...
- `q_H`: a simple draw distribution over `[h_min, h_max]` matching the R reference convention (uniform plus focal-point smoothing is acceptable; documented in the MNL sidecar).
- `q_W`: a draw distribution over `[w_min, w_max]` (uniform is the R reference baseline; an empirical distribution may be substituted with explicit documentation).
...
8. **New YAML spec file `estimation_spec_continuous_enhanced_M0.yaml`** in `scripts/enhanced/`, encoding §22.

9. **New script `scripts/diagnostics/run_recovery_test_continuous_enhanced.py`** (stub allowed at audit time; full implementation in patch plan).
...
**Spec name:** `estimation_spec_continuous_enhanced_M0.yaml`
...
11. **Recovery harness.** Verify or scaffold `scripts/diagnostics/run_recovery_test_continuous_enhanced.py`. Do not run; only verify the entry point.
...
**Save this file as `RURO_model_spec_contract_v2_continuous_enhanced.md`** in the project root or `docs/`. Use it as input to the next Claude Code task: produce `RURO_code_contract_audit_v2.md` against the current `scripts/enhanced/` codebase.