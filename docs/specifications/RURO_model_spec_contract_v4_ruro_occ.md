# RURO Model-Specification Contract v4: continuous-RURO Occupation (EUROMOD-grounded)

**File to save as:** `RURO_model_spec_contract_v4_ruro_occ.md`
**Supersedes:** `RURO_model_spec_contract_v3_ruro_occ.md`. Sections of v3 that are not contradicted here remain valid; in case of conflict, v4 wins.
**Predecessors retained for traceability:** v1 (`RURO_model_spec_contract_v1.md`), v2 (`RURO_model_spec_contract_v2_continuous_enhanced.md`), v3 (`RURO_model_spec_contract_v3_ruro_occ.md`).
...
v3 locked occupation as the third opportunity layer of the enhanced continuous RURO model and ruled out `lindi`-based industry at `M0`. v4 keeps that decision and tightens three things that v3 left implicit:
...
## 2. Why the target is enhanced continuous RURO

the R reference implementation (`ruro/Ruro_estimation_H.Rmd`, `ruro/Ruro_estimation_new.Rmd`, `ruro/Ruro_functions_EMRWS.R`) defines the choice index as
...
- **Working**: `(h, w, loc)`, where `h ∈ [h_min, h_max]` and `w ∈ [w_min, w_max]` are continuous draws (the R reference's uniform proposal is acceptable; an empirical proposal may be substituted with documentation in `mnlmeta.json`), and `loc ∈ {1, …, 9}` (or the corresponding `loc4 ∈ {1, …, 4}`) is drawn from a documented occupation proposal `q_Occ`.
...
Estimator: `enh_RURO_estimate_FR.py --group joint --solver gamspy-conopt --vectorized --spec-config estimation_spec_ruro_occ_M0.yaml`.
...
10. **New YAML spec `scripts/enhanced/estimation_spec_ruro_occ_M0.yaml`**, encoding §24.

11. **New diagnostics script `scripts/diagnostics/run_recovery_test_ruro_occ.py`** (stub allowed at audit time; full implementation in the patch plan).
...
continuous-RURO simulation recovery on France-shaped data:
...
**Spec name:** `estimation_spec_ruro_occ_M0.yaml`
...
12. **Recovery harness.** Verify or scaffold `scripts/diagnostics/run_recovery_test_ruro_occ.py`. Do not run; verify the entry point.
...
**Save this contract as `RURO_model_spec_contract_v4_ruro_occ.md`** in the project root or `docs/`. The next Claude Code task is to produce `RURO_code_contract_audit_v4.md` against the current `scripts/enhanced/` codebase. The patch plan is a separate step.
