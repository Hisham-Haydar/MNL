...
market-opportunity block (corrected GSUR, seven NUTS-1 region
dummies, gender-on-arrival, intercept); hours block (gender-
...
NUTS-1 region dummies, gender-on-arrival, intercept. The
...
`pexp_male`, `pexp_female`, `region_nuts1`, `gsur_male`, `gsur_female`,
...
median, all seven NUTS-1 region dummies at the reference category
(modal region across the sample), gender-on-arrival held or
...
The immediate task following this memo is the *welfare scaffolding
implementation prompt*, to be executed in Claude Code Sonnet
(local codebase, code creation), not in Claude Project chat.

Tool: Claude Code Sonnet.

Inputs to the implementation prompt:
...
- `Prompts/JMP_ability_vs_opportunity_framework_v1.md` (the
  framework memo's C1–C9 code contract, which this memo extends).
- `scripts/enhanced/specifications/estimation_spec_ruro_occ_M1_clean.yaml`
  (the M1-clean YAML; the scaffolding reads its opportunity-block
  and ability-block covariate lists).
- `scripts/enhanced/specifications/estimation_spec_ruro_occ_M1_naive.yaml`
  (the M1-naive YAML).

Output deliverables of the implementation prompt:
- The `scripts/welfare/` module tree per §28.
- The unit tests under `scripts/welfare/tests/` per §28.
- A reference welfare configuration file
  `scripts/welfare/configs/welfare_config_m1_clean_primary.yaml`
  per §22.
- A reference welfare configuration file
  `scripts/welfare/configs/welfare_config_m1_naive_robustness.yaml`
  per §23.
- An implementation report
  `Results/RURO_welfare_scaffold_implementation_report_v1.md`
  documenting which modules were created, which tests pass, which
  diagnostics fire on a dry-run invocation, and any deviation from
  this design.

The implementation prompt does not run the welfare decomposition
on M1-clean or M1-naive. It produces the scaffolding code and
verifies, by unit tests and dry-run invocation, that the
scaffolding meets the design. The welfare-decomposition computation
is authorised by a subsequent prompt that postdates the welfare-
scaffolding verdict.

The recommended parallel chat task in Claude Project chat is the
welfare-scaffolding *audit specification*, which articulates the
verdict criteria that the implementation will be evaluated
against (analogous to the M1-clean design memo §22 SA1 acceptance
rule). The audit specification can be drafted in parallel with the
implementation; its outputs are the verdict criteria, not the
implementation itself.

Items explicitly not authorised by this memo or the subsequent
...
*Pending under separate verdicts*: the welfare-scaffolding
implementation prompt and its verdict, gated on this memo; the
welfare-decomposition computation, gated on the welfare-scaffolding
