...
seven EUROMOD `drgn1` region indicators / old-region aggregation
indicators (`reg2`-`reg8`, with `reg1` as the reference category),
and the gender-on-arrival coefficient under the chosen
gender-attribution rule. The "median" opportunity covariates are:
median GSUR, modal `drgn1` old-region aggregation (reference
category `reg1`), and gender held at its actual value (under the
M1-clean default, which treats gender-on-arrival as opportunity
under the G4 A3 attribution rule).
...
block contains the corrected GSUR, the seven EUROMOD `drgn1`
region indicators / old-region aggregation indicators
(`reg2`-`reg8`, with `reg1` as the reference category),
...
reference values: median GSUR, modal EUROMOD `drgn1` old-region
aggregation (with `reg2`-`reg8` set to zero and `reg1` as the
reference category), and — under the gender attribution rule —
gender-on-arrival held or equalised
