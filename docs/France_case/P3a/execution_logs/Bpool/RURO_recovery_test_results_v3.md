...
> **Mechanism correction (2026-05-29, post-publication):** §2.3 and the "Critical reframe" paragraph below originally attributed CONOPT's escape to "analytical Hessian seeing through the BFGS approximation." A follow-up scipy-Newton benchmark on singles male 766 (`benchmark_scipy_newton_singles_male.log`) **falsifies that explanation**: trust-ncg and Newton-CG with **finite-difference Hessian (real curvature info)** both converge to the **exact same LL = −9737.31** as L-BFGS-B (gap to CONOPT = 7235.55). Three independent scipy methods, three algorithm classes (LBFGS / trust-region NCG / line-search NCG), one with quasi-Newton approximation and two with real curvature — all converge to the same local maximum to 4 decimal places. The mechanism is therefore **NOT approximation quality; it is multi-basin LL structure**: the LL has a real local maximum near θ\* at LL = −9737, and a higher (global, per current evidence) local maximum elsewhere at LL = −2501. Gradient-based descent from θ\* walks to the nearer trap; CONOPT's GRG+SQP algorithm class takes wider exploration steps (active-set-aware bound-projection moves) that traverse the basin barrier. Outcome (i) on identification still stands; the spec is identified, the global optimum recovers all signs correct under CONOPT. But the **multi-basin structure means gradient-only solvers from θ\* are not reliable for production estimation on this spec class** without multistart or a smarter start strategy. See `RURO_solver_multibasin_findings_v1.md` (to follow) for the full benchmark + package recommendation.
...
The findings in the following documents describe **what gradient-based solvers (trust-constr, L-BFGS-B, trust-ncg, Newton-CG) find from θ\* as starting point**. The LL surface has a real local maximum at LL ≈ −9737 (singles male) / ≈ −60,708 (couples-full trust-constr-bfgs trapped value) near θ\*'s basin, and a different higher local maximum at LL ≈ −2501 / −10,862 reachable from CONOPT. The original docs' diagnosis of "structural multi-modality" was qualitatively correct (the LL has multiple local maxima) but the **mechanism attribution** ("trust-constr's BFGS approximation breakdown") was wrong — three independent scipy methods including two with finite-difference Hessian (real curvature info) all converge to the same near-θ\* trap to 4 decimals (see `benchmark_scipy_newton_singles_male.log`). The corrections below describe both the original framing and the mechanically correct understanding.
...
| `RURO_recovery_test_results_v2.md` | "non-convexity in the couples leisure block on the 9×9 joint-leisure grid"; recommends parsimonising the leisure block | **Qualitatively right** (LL is multi-basin) but **wrong about scope** — the multi-basin structure is in the wage/E/leisure block jointly, not specifically the couples leisure 9×9 grid. The couples leisure interior under CONOPT recovers 9/9 with correct signs at the global local max. Parsimonisation recommendation should not be acted on; the flag fix (commit `099e5c4`) addressed the LH-bound-saturation symptom, but the multi-basin LL remains a real property requiring multistart or smart-start for any gradient-based solver. |
| `RURO_recovery_test_results_singles_v1.md` | Box-Cox theta_l multi-modality; theta_l sign-flip at the wrong attractor; chosen-row working_lh diagnosis (correct, led to commit `099e5c4`) | The chosen-row LH diagnosis was correct. The "multi-modality" framing was **qualitatively correct** (LL has multiple local maxima); the **theta_l sign-flip at the trapped basin is real** but escapable by CONOPT. The theta_l sign-constraint experiment was correctly rejected as ineffective for the same reason: constraining theta_l doesn't help gradient solvers escape the basin; only switching solvers does. |
| `RURO_recovery_test_results_v3_singles_*_raw.md` and the trust-constr couples-full raw | "Outcome (ii) with structural non-convexity" | The non-convexity at LL ≈ −9737 (singles male) is a **real local maximum of the LL** that any gradient-based solver from θ\* converges to (corrected 2026-05-29: trust-ncg + Newton-CG with FD Hessian both confirm this; not a BFGS approximation issue). CONOPT escapes via GRG+SQP active-set steps that cross the basin barrier; gradient-based methods do not. Outcome (i) on identification still stands at the GLOBAL local max (LL ≈ −2501) which CONOPT reaches. |
