...
class PrecomputedDataCouples:
    ...
    l_male: np.ndarray             # normalized leisure for male
    age_norm_male: np.ndarray      # demeaned age male
    age_norm2_male: np.ndarray     # demeaned age² male
    educL_male: np.ndarray         # low education male
    educH_male: np.ndarray         # high education male
    
    # Female partner leisure and demographics
    l_female: np.ndarray             # normalized leisure for female
    age_norm_female: np.ndarray      # demeaned age female
    age_norm2_female: np.ndarray     # demeaned age² female
    educL_female: np.ndarray         # low education female
    educH_female: np.ndarray         # high education female
    ...
    pexp2_male: np.ndarray         # pre-computed pexp^2
    
    # Wage-related - female
    log_wage_f: np.ndarray
    pexp_female: np.ndarray
    pexp2_female: np.ndarray         # pre-computed pexp^2
...
def precompute_data_couples(df: pd.DataFrame) -> PrecomputedDataCouples:
    ...
    if c is not None:
        ...
        if ids is not None:
            ...
            if not c_varies:
                ...
                c = None
      # If no valid consumption, use ils_dispy_em from EUROMOD
    if c is None:
        # For couples, use ils_dispy_em directly (household aggregate from EUROMOD)
        # This is the disposable income for the household (coupM + coupF)
        # User specification: c_ij = ils_dispy_coupM + ils_dispy_coupF
        # The column ils_dispy_em already contains this household aggregate

        # DIAGNOSTIC: Check if column exists
        if "ils_dispy_em" in df.columns:
            LOGGER.info("✓ ils_dispy_em column FOUND in couples DataFrame")
            c_raw = df["ils_dispy_em"].to_numpy(dtype=np.float64)
            LOGGER.info(f"  Raw values: mean={c_raw.mean():.3f}, std={c_raw.std():.3f}, n_unique={len(np.unique(c_raw))}, min={c_raw.min():.3f}, max={c_raw.max():.3f}")
            c = c_raw
        else:
            LOGGER.error("❌ ils_dispy_em column NOT FOUND in couples DataFrame!")
            LOGGER.error(f"  Available columns ({len(df.columns)}): {df.columns.tolist()[:20]}...")
            c = safe_get("ruro_consumption", 0.0)
            LOGGER.warning(f"  Using ruro_consumption fallback: mean={c.mean():.3f}")

        if np.all(c == 0):
            LOGGER.warning("Consumption is all zeros! Trying alternate columns...")
            c = safe_get("consumption", 0.0)

        # Normalize by mean disposable income from observed jobs
        c = c / MEAN_DISPY_NORM

        LOGGER.info(f"Final couples consumption (normalized): mean={c.mean():.3f}, std={c.std():.3f}, n_unique={len(np.unique(c))}")
    ...
    # Leisure - male
    l_male = None
    if "leis_util_male" in df.columns and not df["leis_util_male"].isna().all():
        l_male = df["leis_util_male"].to_numpy(dtype=np.float64)
    if l_male is None and "leisure_male" in df.columns:
        l_male = df["leisure_male"].to_numpy(dtype=np.float64) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    if l_male is None and "hours_male" in df.columns:
        hours_male = df["hours_male"].to_numpy(dtype=np.float64)
        l_male = (TOTAL_LEISURE_HOURS - hours_male) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    if l_male is None:
        l_male = np.ones(n, dtype=np.float64)
    l_male = np.ascontiguousarray(np.clip(l_male, 1e-6, None), dtype=np.float64)
    
    # Leisure - female
    l_female = None
    if "leis_util_female" in df.columns and not df["leis_util_female"].isna().all():
        l_female = df["leis_util_female"].to_numpy(dtype=np.float64)
    if l_female is None and "leisure_female" in df.columns:
        l_female = df["leisure_female"].to_numpy(dtype=np.float64) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    if l_female is None and "hours_female" in df.columns:
        hours_female = df["hours_female"].to_numpy(dtype=np.float64)
        l_female = (TOTAL_LEISURE_HOURS - hours_female) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    if l_female is None:
        l_female = np.ones(n, dtype=np.float64)
    l_female = np.ascontiguousarray(np.clip(l_female, 1e-6, None), dtype=np.float64)
      # Age (normalized) - male/female
    age_norm_male = safe_get("age_norm_male")
    age_norm2_male = safe_get("age_norm2_male")
    
    age_norm_female = safe_get("age_norm_female")
    age_norm2_female = safe_get("age_norm2_female")
    
    # Education
    educL_male = safe_get("educL_male")
    educH_male = safe_get("educH_male")
    educL_female = safe_get("educL_female")
    educH_female = safe_get("educH_female")
    
    # Children (household-level, use total n_children)
    n_children = safe_get("n_children")
    
    # Hours indicators - male
    working_male = safe_get("working_male")
    if working_male.sum() == 0 and "hours_male" in df.columns:
        hours_male = pd.to_numeric(df["hours_male"], errors="coerce").fillna(0).to_numpy()
        working_male = np.ascontiguousarray((hours_male > 0).astype(np.float64))
    working_pt1_male = safe_get("working_pt1_male")
    working_pt2_male = safe_get("working_pt2_male")
    working_ft_male = safe_get("working_ft_male")
    if working_pt1_male.sum() == 0 and "hours_male" in df.columns:
        hours_male = pd.to_numeric(df["hours_male"], errors="coerce").fillna(0).to_numpy()
        working_pt1_male = np.ascontiguousarray(((hours_male >= 18.5) & (hours_male <= 21.5)).astype(np.float64))
        working_pt2_male = np.ascontiguousarray(((hours_male >= 29.5) & (hours_male <= 30.5)).astype(np.float64))
        working_ft_male = np.ascontiguousarray(((hours_male >= 37.5) & (hours_male <= 40.5)).astype(np.float64))
    gsur_male = safe_get("gsur_male")
    if gsur_male.sum() == 0:
        gsur_male = safe_get("gsur")
    
    # Hours indicators - female
    working_female = safe_get("working_female")
    if working_female.sum() == 0 and "hours_female" in df.columns:
        hours_female = pd.to_numeric(df["hours_female"], errors="coerce").fillna(0).to_numpy()
        working_female = np.ascontiguousarray((hours_female > 0).astype(np.float64))
    working_pt1_female = safe_get("working_pt1_female")
    working_pt2_female = safe_get("working_pt2_female")
    working_ft_female = safe_get("working_ft_female")
    if working_pt1_female.sum() == 0 and "hours_female" in df.columns:
        hours_female = pd.to_numeric(df["hours_female"], errors="coerce").fillna(0).to_numpy()
        working_pt1_female = np.ascontiguousarray(((hours_female >= 18.5) & (hours_female <= 21.5)).astype(np.float64))
        working_pt2_female = np.ascontiguousarray(((hours_female >= 29.5) & (hours_female <= 30.5)).astype(np.float64))
        working_ft_female = np.ascontiguousarray(((hours_female >= 37.5) & (hours_female <= 40.5)).astype(np.float64))
    gsur_female = safe_get("gsur_female")
    if gsur_female.sum() == 0:
        gsur_female = safe_get("gsur")
    ...
    if "pexp_male" in df.columns:
        pexp_male = pd.to_numeric(df["pexp_male"], errors="coerce").fillna(0).to_numpy(dtype=np.float64)
    else:
        pexp_male = np.zeros(n, dtype=np.float64)
    pexp_male = np.ascontiguousarray(pexp_male)
    pexp2_male = np.ascontiguousarray(pexp_male ** 2)
    ...
    if "pexp_female" in df.columns:
        pexp_female = pd.to_numeric(df["pexp_female"], errors="coerce").fillna(0).to_numpy(dtype=np.float64)
    else:
        pexp_female = np.zeros(n, dtype=np.float64)
    pexp_female = np.ascontiguousarray(pexp_female)
    pexp2_female = np.ascontiguousarray(pexp_female ** 2)
    ...
    return PrecomputedDataCouples(
        c=c, l_male=l_male, age_norm_male=age_norm_male, age_norm2_male=age_norm2_male,
        educL_male=educL_male, educH_male=educH_male,
        l_female=l_female, age_norm_female=age_norm_female, age_norm2_female=age_norm2_female,
        educL_female=educL_female, educH_female=educH_female,
        n_children=n_children,
        working_male=working_male, working_pt1_male=working_pt1_male, working_pt2_male=working_pt2_male,
        working_ft_male=working_ft_male, gsur_male=gsur_male,
        working_female=working_female, working_pt1_female=working_pt1_female, working_pt2_female=working_pt2_female,
        working_ft_female=working_ft_female, gsur_female=gsur_female,
        log_wage_m=log_wage_m, pexp_male=pexp_male, pexp2_male=pexp2_male,
        log_wage_f=log_wage_f, pexp_female=pexp_female, pexp2_female=pexp2_female,
...
class PrefParamsCouples:
    ...
    CORRECTED SPECIFICATION (male=5 + female=6 + shared=5 params):
    - age_norm = dag - mean(dag) (demeaned, not log)
    - Single n_children replaces ch0_3, ch4_6, ch7_9 (ONLY for females!)
    - No region effects on preferences
    - Includes cross-leisure interaction (beta_interact * l_male * l_female)

    Box-Cox utility structure:
        u = (male leisure terms) * (l_male^θ_lm - 1)/θ_lm
          + (female leisure terms) * (l_female^θ_lf - 1)/θ_lf
          + β_c * ((c_m + c_f)^θ_c - 1)/θ_c
          + β_interact * l_male * l_female
    """
    # Male leisure coefficients (5 params - NO n_children!)
    beta_lm0: float = 1.0
    ...
    # Leisure-leisure interaction term
    beta_interact: float = 0.0  # interaction between male and female leisure
...
def ff_calc_util_couples(
    ...
    SIMPLIFIED SPECIFICATION:
        u = (β_lm0 + β_lm_age_norm*age_norm_male + β_lm_age_norm2*age_norm²_m
             + β_lm_n_children*n_children
             + β_lm_educL*educL_male + β_lm_educH*educH_male)
            * (l_male^θ_lm - 1)/θ_lm
          + (β_lf0 + ... female terms ...)
            * (l_female^θ_lf - 1)/θ_lf
          + β_c * ((c_m + c_f)^θ_c - 1)/θ_c
    ...
    if "leis_util_male" in df.columns:
        l_male = df["leis_util_male"].to_numpy()
    elif "l_norm_m" in df.columns:
        l_male = df["l_norm_m"].to_numpy()
    elif "hours_male" in df.columns:
        hours_male = pd.to_numeric(df["hours_male"], errors="coerce").fillna(0).to_numpy()
        l_male = (TOTAL_LEISURE_HOURS - hours_male) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    else:
        l_male = np.ones(n)
    ...
    if "leis_util_female" in df.columns:
        l_female = df["leis_util_female"].to_numpy()
    elif "l_norm_f" in df.columns:
        l_female = df["l_norm_f"].to_numpy()
    elif "hours_female" in df.columns:
        hours_female = pd.to_numeric(df["hours_female"], errors="coerce").fillna(0).to_numpy()
        l_female = (TOTAL_LEISURE_HOURS - hours_female) / (TOTAL_LEISURE_HOURS - MEAN_LHW_NORM)
    else:
        l_female = np.ones(n)
    ...
    l_male = np.clip(l_male, 1e-6, None)
    l_female = np.clip(l_female, 1e-6, None)
    c_total = np.clip(c_total, 1e-6, None)
    
    # -------------------------------------------------------------------------
    # Covariates for male partner (simplified: demeaned age)
    # -------------------------------------------------------------------------
    age_norm_male = _get_col(df, "age_norm_male", 0.0)
    age_norm2_male = _get_col(df, "age_norm2_male", 0.0)
    if age_norm2_male.sum() == 0:
        age_norm2_male = age_norm_male ** 2
    
    educL_male = _get_col(df, "educL_male", 0.0)
    educH_male = _get_col(df, "educH_male", 0.0)
    
    # -------------------------------------------------------------------------
    # Covariates for female partner (simplified: demeaned age)
    # -------------------------------------------------------------------------
    age_norm_female = _get_col(df, "age_norm_female", 0.0)
    age_norm2_female = _get_col(df, "age_norm2_female", 0.0)
    if age_norm2_female.sum() == 0:
        age_norm2_female = age_norm_female ** 2
    
    educL_female = _get_col(df, "educL_female", 0.0)
    educH_female = _get_col(df, "educH_female", 0.0)
    ...
    # -------------------------------------------------------------------------
    # Build male leisure coefficient (CORRECTED - NO n_children!)
    # -------------------------------------------------------------------------
    beta_leisure_m = (
        pref.beta_lm0
        + pref.beta_lm_age_norm * age_norm_male
        + pref.beta_lm_age_norm2 * age_norm2_male
        # NO n_children for males!
        + pref.beta_lm_educL * educL_male
        + pref.beta_lm_educH * educH_male
    )

    # -------------------------------------------------------------------------
    # Build female leisure coefficient (WITH n_children)
    # -------------------------------------------------------------------------
    beta_leisure_f = (
        pref.beta_lf0
        + pref.beta_lf_age_norm * age_norm_female
        + pref.beta_lf_age_norm2 * age_norm2_female
        + pref.beta_lf_n_children * n_children  # n_children only for females
        + pref.beta_lf_educL * educL_female
        + pref.beta_lf_educH * educH_female
    )
    
    # -------------------------------------------------------------------------
    # Box-Cox transforms
    # -------------------------------------------------------------------------
    l_m_bc = boxcox_transform(l_male, pref.theta_lm)
    l_f_bc = boxcox_transform(l_female, pref.theta_lf)
    ...
    u = (
        ...
        + pref.beta_interact * l_male * l_female # Leisure-leisure interaction term
    )
...
def ff_calc_hopp_couples(
    ...
    For couples, the opportunity density is the sum of male and female components:
        ...
        hopp = (hopp_male_terms using working_male, educL_male, gsur_male, etc.)
             + (hopp_female_terms using working_female, educL_female, gsur_female, etc.)
    ...
    # -------------------------------------------------------------------------
    working_male = _get_col(df, "working_male", 0.0)
    working_pt1_male = _get_col(df, "working_pt1_male", 0.0)
    working_pt2_male = _get_col(df, "working_pt2_male", 0.0)
    working_ft_male = _get_col(df, "working_ft_male", 0.0)
    
    # If not present, derive from hours_male
    if "working_male" not in df.columns and "hours_male" in df.columns:
        hours_male = pd.to_numeric(df["hours_male"], errors="coerce").fillna(0).to_numpy()
        working_male = (hours_male > 0).astype(float)
        working_pt1_male = ((hours_male >= 18.5) & (hours_male <= 21.5)).astype(float)
        working_pt2_male = ((hours_male >= 29.5) & (hours_male <= 30.5)).astype(float)
        working_ft_male = ((hours_male >= 37.5) & (hours_male <= 40.5)).astype(float)
    
    gsur_male = _get_col(df, "gsur_male", 0.0)
    if gsur_male.sum() == 0:
        gsur_male = _get_col(df, "gsur", 0.0)  # Fallback to household gsur
    educL_male = _get_col(df, "educL_male", 0.0)
    educH_male = _get_col(df, "educH_male", 0.0)
    ...
    # -------------------------------------------------------------------------
    working_female = _get_col(df, "working_female", 0.0)
    working_pt1_female = _get_col(df, "working_pt1_female", 0.0)
    working_pt2_female = _get_col(df, "working_pt2_female", 0.0)
    working_ft_female = _get_col(df, "working_ft_female", 0.0)
    
    if "working_female" not in df.columns and "hours_female" in df.columns:
        hours_female = pd.to_numeric(df["hours_female"], errors="coerce").fillna(0).to_numpy()
        working_female = (hours_female > 0).astype(float)
        working_pt1_female = ((hours_female >= 18.5) & (hours_female <= 21.5)).astype(float)
        working_pt2_female = ((hours_female >= 29.5) & (hours_female <= 30.5)).astype(float)
        working_ft_female = ((hours_female >= 37.5) & (hours_female <= 40.5)).astype(float)
    
    gsur_female = _get_col(df, "gsur_female", 0.0)
    if gsur_female.sum() == 0:
        gsur_female = _get_col(df, "gsur", 0.0)
    educL_female = _get_col(df, "educL_female", 0.0)
    educH_female = _get_col(df, "educH_female", 0.0)
    ...
    # -------------------------------------------------------------------------
    h_opp_m = (
        hopp_m.beta_work * working_male
        + hopp_m.beta_pt1 * working_pt1_male
        + hopp_m.beta_pt2 * working_pt2_male
        + hopp_m.beta_ft * working_ft_male
        + hopp_m.beta_gsur * working_male * gsur_male
        + hopp_m.beta_work_educL * working_male * educL_male
        + hopp_m.beta_work_educH * working_male * educH_male
    )
    
    h_opp_f = (
        hopp_f.beta_work * working_female
        + hopp_f.beta_pt1 * working_pt1_female
        + hopp_f.beta_pt2 * working_pt2_female
        + hopp_f.beta_ft * working_ft_female
        + hopp_f.beta_gsur * working_female * gsur_female
        + hopp_f.beta_work_educL * working_female * educL_female
        + hopp_f.beta_work_educH * working_female * educH_female
    )
...
def ff_calc_wopp_couples(
    ...
    educL_male = _get_col(df, "educL_male", 0.0)
    educH_male = _get_col(df, "educH_male", 0.0)
    pexp_male = _get_col(df, "pexp_male", 0.0)
    ...
    educL_female = _get_col(df, "educL_female", 0.0)
    educH_female = _get_col(df, "educH_female", 0.0)
    pexp_female = _get_col(df, "pexp_female", 0.0)
    ...
    mean_logw_m = (
        wopp_m.beta0
        + wopp_m.beta_educL * educL_male
        + wopp_m.beta_educH * educH_male
        + wopp_m.beta_pexp * pexp_male
        + wopp_m.beta_pexp2 * (pexp_male ** 2)
    )
    
    sigma_m = np.abs(wopp_m.sigma) + 1e-6
    z_m = (log_wage_m - mean_logw_m) / sigma_m
    w_opp_m = -0.5 * z_m**2 - np.log(sigma_m) - log_wage_m
    w_opp_m = np.where(working_male > 0, w_opp_m, 0.0)
    ...
    mean_logw_f = (
        wopp_f.beta0
        + wopp_f.beta_educL * educL_female
        + wopp_f.beta_educH * educH_female
        + wopp_f.beta_pexp * pexp_female
        + wopp_f.beta_pexp2 * (pexp_female ** 2)
    )
    
    sigma_f = np.abs(wopp_f.sigma) + 1e-6
    z_f = (log_wage_f - mean_logw_f) / sigma_f
    w_opp_f = -0.5 * z_f**2 - np.log(sigma_f) - log_wage_f
    w_opp_f = np.where(working_female > 0, w_opp_f, 0.0)
...
def fast_log_likelihood_couples(
    ...
    """
    # -------------------------------------------------------------------------
    # Unpack theta - CORRECTED STRUCTURE
    # -------------------------------------------------------------------------
    # Male leisure preferences [0:5] - NO n_children!
    beta_lm0 = theta[0]
    ...
    # Female leisure preferences [5:11] - WITH n_children
    beta_lf0 = theta[5]
    ...
    # Shared params [11:16] - WITH interaction
    beta_c = theta[11]
    theta_lm = theta[12]
    theta_lf = theta[13]
    theta_c = theta[14]
    beta_interact = theta[15]
    ...
    # -------------------------------------------------------------------------
    # Box-Cox transforms
    lm_bc = _fast_boxcox(data.l_male, theta_lm)
    lf_bc = _fast_boxcox(data.l_female, theta_lf)
    c_bc = _fast_boxcox(data.c, theta_c)
    
    # Male leisure coefficient (CORRECTED - NO n_children!)
    beta_leisure_m = (
        beta_lm0
        + beta_lm_age_norm * data.age_norm_male
        + beta_lm_age_norm2 * data.age_norm2_male
        # NO n_children for males!
        + beta_lm_educL * data.educL_male
        + beta_lm_educH * data.educH_male
    )

    # Female leisure coefficient (WITH n_children)
    beta_leisure_f = (
        beta_lf0
        + beta_lf_age_norm * data.age_norm_female
        + beta_lf_age_norm2 * data.age_norm2_female
        + beta_lf_n_children * data.n_children  # n_children only for females
        + beta_lf_educL * data.educL_female
        + beta_lf_educH * data.educH_female
    )

    # Total utility (WITH interaction term!)
    u = (beta_leisure_m * lm_bc + beta_leisure_f * lf_bc + beta_c * c_bc +
         beta_interact * data.l_male * data.l_female)
    ...
    h_opp_m = (
        ...
        + beta_work_educL_m * data.working_male * data.educL_male
        + beta_work_educH_m * data.working_male * data.educH_male
    )
    ...
    h_opp_f = (
        ...
        + beta_work_educL_f * data.working_female * data.educL_female
        + beta_work_educH_f * data.working_female * data.educH_female
    )
    ...
    if wage_spec == "vw" and len(theta) > 30:
        ...
        mean_logw_m = (
            w_beta0_m
            + w_educL_m * data.educL_male + w_educH_m * data.educH_male
            + w_pexp_m * data.pexp_male + w_pexp2_m * data.pexp2_male
        )
        z_m = (data.log_wage_m - mean_logw_m) / sigma_m
        w_opp_m = np.where(data.working_male > 0, 
        ...
        mean_logw_f = (
            w_beta0_f
            + w_educL_f * data.educL_female + w_educH_f * data.educH_female
            + w_pexp_f * data.pexp_female + w_pexp2_f * data.pexp2_female
        )
        z_f = (data.log_wage_f - mean_logw_f) / sigma_f
        w_opp_f = np.where(data.working_female > 0,
...
def fast_neg_ll_with_grad_couples(
    ...
    CORRECTED SPECIFICATION (matching fast_log_likelihood_couples):
    - Uses demeaned age (age_norm) instead of log(age)
    - Single n_children variable ONLY for females (NOT males!)
    - No region effects on preferences or hours opportunity
    - No year dummies on wages
    - Includes leisure-leisure interaction term

    Parameter layout (30 for fw, 42 for vw):
    [0:5]    - Male leisure preferences (5: NO n_children!)
    [5:11]   - Female leisure preferences (6: WITH n_children)
    [11:16]  - Shared: beta_c, theta_lm, theta_lf, theta_c, beta_interact
    [16:23]  - Male hours opportunity (7 params)
    [23:30]  - Female hours opportunity (7 params)
    [30:36]  - Male wage opportunity (6 params, vw only)
    [36:42]  - Female wage opportunity (6 params, vw only)
    ...
    lf_bc = _fast_boxcox(data.l_female, theta_lf)
    c_bc = _fast_boxcox(data.c, theta_c)
    dlm_bc_dtheta = _fast_d_boxcox_dtheta(data.l_male, theta_lm)
    dlf_bc_dtheta = _fast_d_boxcox_dtheta(data.l_female, theta_lf)
    dc_bc_dtheta = _fast_d_boxcox_dtheta(data.c, theta_c)
    
    # -------------------------------------------------------------------------
    # Utility computation (CORRECTED)
    # -------------------------------------------------------------------------
    beta_leisure_m = (
        beta_lm0
        + beta_lm_age_norm * data.age_norm_male
    ...
    # -------------------------------------------------------------------------
    # Utility derivatives (16 params) - CORRECTED STRUCTURE
    # -------------------------------------------------------------------------
    du_dtheta = np.empty((n, 16), dtype=np.float64)

    # Male leisure params [0:5] - NO n_children!
    du_dtheta[:, 0] = lm_bc                          # beta_lm0
    du_dtheta[:, 1] = data.age_norm_male * lm_bc        # beta_lm_age_norm
    du_dtheta[:, 2] = data.age_norm2_male * lm_bc       # beta_lm_age_norm2
    du_dtheta[:, 3] = data.educL_male * lm_bc           # beta_lm_educL
    du_dtheta[:, 4] = data.educH_male * lm_bc           # beta_lm_educH

    # Female leisure params [5:11] - WITH n_children
    du_dtheta[:, 5] = lf_bc                          # beta_lf0
    du_dtheta[:, 6] = data.age_norm_female * lf_bc        # beta_lf_age_norm
    du_dtheta[:, 7] = data.age_norm2_female * lf_bc       # beta_lf_age_norm2
    du_dtheta[:, 8] = data.n_children * lf_bc        # beta_lf_n_children (females only!)
    du_dtheta[:, 9] = data.educL_female * lf_bc           # beta_lf_educL
    du_dtheta[:, 10] = data.educH_female * lf_bc          # beta_lf_educH

    # Shared params [11:16] - WITH interaction
    du_dtheta[:, 11] = c_bc                             # beta_c
    du_dtheta[:, 12] = beta_leisure_m * dlm_bc_dtheta   # theta_lm
    du_dtheta[:, 13] = beta_leisure_f * dlf_bc_dtheta   # theta_lf
    du_dtheta[:, 14] = beta_c * dc_bc_dtheta            # theta_c
    du_dtheta[:, 15] = data.l_male * data.l_female              # beta_interact (NEW!)
    ...
    # Male hopp [0:7]
    dh_dtheta[:, 0] = data.working_male
    dh_dtheta[:, 1] = data.working_pt1_male
    dh_dtheta[:, 2] = data.working_pt2_male
    dh_dtheta[:, 3] = data.working_ft_male
    dh_dtheta[:, 4] = data.working_male * data.gsur_male
    dh_dtheta[:, 5] = data.working_male * data.educL_male
    dh_dtheta[:, 6] = data.working_male * data.educH_male
    # Female hopp [7:14]
    dh_dtheta[:, 7] = data.working_female
    dh_dtheta[:, 8] = data.working_pt1_female
    dh_dtheta[:, 9] = data.working_pt2_female
    dh_dtheta[:, 10] = data.working_ft_female
    dh_dtheta[:, 11] = data.working_female * data.gsur_female
    dh_dtheta[:, 12] = data.working_female * data.educL_female
    dh_dtheta[:, 13] = data.working_female * data.educH_female
    ...
    if wage_spec == "vw" and len(theta) > 30:
        ...
        mean_logw_m = (
            w_beta0_m
            + w_educL_m * data.educL_male
            + w_educH_m * data.educH_male
        ...
        mean_logw_f = (
            w_beta0_f
            + w_educL_f * data.educL_female
            + w_educH_f * data.educH_female
        ...
        # Male wopp [0:6]
        dw_dtheta[:, 0] = np.where(data.working_male > 0, z_over_sigma_m, 0.0)
        dw_dtheta[:, 1] = np.where(data.working_male > 0, z_over_sigma_m * data.educL_male, 0.0)
        dw_dtheta[:, 2] = np.where(data.working_male > 0, z_over_sigma_m * data.educH_male, 0.0)
        dw_dtheta[:, 3] = np.where(data.working_male > 0, z_over_sigma_m * data.pexp_male, 0.0)
        dw_dtheta[:, 4] = np.where(data.working_male > 0, z_over_sigma_m * data.pexp2_male, 0.0)
        dw_dtheta[:, 5] = np.where(data.working_male > 0, (z_m * z_m - 1.0) / sigma_m, 0.0)
        # Female wopp [6:12]
        dw_dtheta[:, 6] = np.where(data.working_female > 0, z_over_sigma_f, 0.0)
        dw_dtheta[:, 7] = np.where(data.working_female > 0, z_over_sigma_f * data.educL_female, 0.0)
        dw_dtheta[:, 8] = np.where(data.working_female > 0, z_over_sigma_f * data.educH_female, 0.0)
        dw_dtheta[:, 9] = np.where(data.working_female > 0, z_over_sigma_f * data.pexp_female, 0.0)
        dw_dtheta[:, 10] = np.where(data.working_female > 0, z_over_sigma_f * data.pexp2_female, 0.0)
        dw_dtheta[:, 11] = np.where(data.working_female > 0, (z_f * z_f - 1.0) / sigma_f, 0.0)
    else:
        w_opp = 0.0
        dw_dtheta = None
...
def get_n_params_joint(wage_spec: str = "fw") -> Dict[str, int]:
    ...
    n_pref_cou = 16     # couples preferences: male(5) + female(6) + shared(5) with interaction
...
def get_initial_theta_joint(wage_spec: str = "fw") -> np.ndarray:
    ...
    # Couples preferences (16) - CORRECTED: male(5) + female(6) + shared(5)
    pref_cou = [
        # Male leisure (5) - NO n_children!
        1.0,   # beta_l0_m
        0.0,   # beta_l_age_norm_m
        0.0,   # beta_l_age_norm2_m
        0.0,   # beta_l_n_children_m  # FIXME: REMOVE THIS - males don't have n_children!
        0.0,   # beta_l_educL_m
        0.0,   # beta_l_educH_m
        # Female leisure (6) - WITH n_children
        1.0,   # beta_l0_f
        ...
        0.0,   # beta_l_educL_f
        0.0,   # beta_l_educH_f
        # Shared (5) - WITH interaction term
        1.0,   # beta_c
        0.5,   # theta_l_m
        0.5,   # theta_l_f
        0.5,   # theta_c
        # FIXME: ADD beta_interact = 0.0 here!
    ]
...
def get_param_names_joint(wage_spec: str = "fw") -> List[str]:
    ...
    # Couples preferences (16) - CORRECTED: male(5) + female(6) + shared(5)
    names += [
        # Male (5) - FIXME: REMOVE beta_l_n_children_m!
        ...
        # Shared (5) - FIXME: ADD "cou.pref.beta_interact" here!
...
def _extract_group_params_from_joint(
    ...
    if group == "sm":
        ...
        # Couples: prefs(16: male5+female6+shared5) + hopp_m(7) + hopp_f(7) + wopp_m(6) + wopp_f(6)
...
def unpack_theta_couples(theta: np.ndarray, wage_spec: str = "fw") -> Tuple:
    ...
    """
    # Preferences (16 params total) - CORRECTED STRUCTURE
    pref = PrefParamsCouples(
        # Male leisure (5) [0:5] - NO n_children!
        beta_lm0=theta[0],
        beta_lm_age_norm=theta[1],
        beta_lm_age_norm2=theta[2],
        beta_lm_educL=theta[3],
        beta_lm_educH=theta[4],
        # Female leisure (6) [5:11] - WITH n_children
        beta_lf0=theta[5],
        beta_lf_age_norm=theta[6],
        beta_lf_age_norm2=theta[7],
        beta_lf_n_children=theta[8],
        beta_lf_educL=theta[9],
        beta_lf_educH=theta[10],
        # Shared (5) [11:16] - WITH interaction
        beta_c=theta[11],
        theta_lm=theta[12],
        theta_lf=theta[13],
        theta_c=theta[14],
...
def analytical_gradient_couples(
    ...
    dl_m_bc_dtheta = d_boxcox_dtheta(l_male, pref.theta_lm)
    dl_f_bc_dtheta = d_boxcox_dtheta(l_female, pref.theta_lf)
    dc_bc_dtheta = d_boxcox_dtheta(c_total, pref.theta_c)
    
    # Covariates
    age_norm_male = _get_col(df, "age_norm_male", 0.0)
    age_norm2_male = _get_col(df, "age_norm2_male", age_norm_male ** 2)
    age_norm_female = _get_col(df, "age_norm_female", 0.0)
    age_norm2_female = _get_col(df, "age_norm2_female", age_norm_female ** 2)
    n_children = _get_col(df, "n_children", 0.0)
    educL_male = _get_col(df, "educL_male", 0.0)
    educH_male = _get_col(df, "educH_male", 0.0)
    educL_female = _get_col(df, "educL_female", 0.0)
    ...
    du_dpref[:, 1] = age_norm_male * l_m_bc      # beta_lm_age_norm
    du_dpref[:, 2] = age_norm2_male * l_m_bc     # beta_lm_age_norm2
    du_dpref[:, 3] = n_children * l_m_bc      # beta_lm_n_children
    du_dpref[:, 4] = educL_male * l_m_bc         # beta_lm_educL
    du_dpref[:, 5] = educH_male * l_m_bc         # beta_lm_educH
    
    # Female leisure params [6:12]
    du_dpref[:, 6] = l_f_bc                   # beta_lf0
    du_dpref[:, 7] = age_norm_female * l_f_bc      # beta_lf_age_norm
    du_dpref[:, 8] = age_norm2_female * l_f_bc     # beta_lf_age_norm2
    du_dpref[:, 9] = n_children * l_f_bc      # beta_lf_n_children
    du_dpref[:, 10] = educL_female * l_f_bc        # beta_lf_educL
    du_dpref[:, 11] = educH_female * l_f_bc        # beta_lf_educH
    ...
    working_pt2_male = _get_col(df, "working_pt2_male", 0.0)
    working_ft_male = _get_col(df, "working_ft_male", 0.0)
    gsur_male = _get_col(df, "gsur_male", _get_col(df, "gsur", 0.0))
    
    working_female = _get_col(df, "working_female", 0.0)
    working_pt1_female = _get_col(df, "working_pt1_female", 0.0)
    working_pt2_female = _get_col(df, "working_pt2_female", 0.0)
    working_ft_female = _get_col(df, "working_ft_female", 0.0)
    gsur_female = _get_col(df, "gsur_female", _get_col(df, "gsur", 0.0))
    
    dh_dtheta = np.zeros((n, 14), dtype=float)
    # Male hopp [0:7]
    dh_dtheta[:, 0] = working_male
    dh_dtheta[:, 1] = working_pt1_male
    dh_dtheta[:, 2] = working_pt2_male
    dh_dtheta[:, 3] = working_ft_male
    dh_dtheta[:, 4] = working_male * gsur_male
    dh_dtheta[:, 5] = working_male * educL_male
    dh_dtheta[:, 6] = working_male * educH_male
    # Female hopp [7:14]
    dh_dtheta[:, 7] = working_female
    dh_dtheta[:, 8] = working_pt1_female
    dh_dtheta[:, 9] = working_pt2_female
    dh_dtheta[:, 10] = working_ft_female
    dh_dtheta[:, 11] = working_female * gsur_female
    dh_dtheta[:, 12] = working_female * educL_female
    dh_dtheta[:, 13] = working_female * educH_female
    ...
    if wage_spec == "vw":
        ...
        # Male wopp [0:6]
        dw_dtheta[:, 0] = np.where(working_male > 0, z_over_sigma_m, 0)
        dw_dtheta[:, 1] = np.where(working_male > 0, z_over_sigma_m * educL_male, 0)
        dw_dtheta[:, 2] = np.where(working_male > 0, z_over_sigma_m * educH_male, 0)
        dw_dtheta[:, 3] = np.where(working_male > 0, z_over_sigma_m * pexp_male, 0)
        dw_dtheta[:, 4] = np.where(working_male > 0, z_over_sigma_m * pexp_male**2, 0)
        dw_dtheta[:, 5] = np.where(working_male > 0, (z_m**2 - 1) / sigma_m, 0)
        # Female wopp [6:12]
        dw_dtheta[:, 6] = np.where(working_female > 0, z_over_sigma_f, 0)
        dw_dtheta[:, 7] = np.where(working_female > 0, z_over_sigma_f * educL_female, 0)
        dw_dtheta[:, 8] = np.where(working_female > 0, z_over_sigma_f * educH_female, 0)
        dw_dtheta[:, 9] = np.where(working_female > 0, z_over_sigma_f * pexp_female, 0)
        dw_dtheta[:, 10] = np.where(working_female > 0, z_over_sigma_f * pexp_female**2, 0)
        dw_dtheta[:, 11] = np.where(working_female > 0, (z_f**2 - 1) / sigma_f, 0)
    else:
        w_opp = np.zeros(n)
...
def fast_neg_ll_with_grad_joint(
    ...
    Couples standalone expects (29 fw, 41 vw):
    - Male prefs: 5 (beta_lm0, age_norm, age_norm2, educL, educH) - NO n_children!
    - Female prefs: 6 (beta_lf0, age_norm, age_norm2, n_children, educL, educH)
    - Shared prefs: 5 (beta_c, theta_lm, theta_lf, theta_c, beta_interact)
    - Male hopp: 7
    - Female hopp: 7
    - Male wopp: 6 (vw only)
    ...
    # ========================================================================
    # DIAGNOSTIC LOGGING - Helps identify gradient issues
    # ========================================================================
    # Print LL contributions (note: these are NEGATIVE LL values from the functions)
    try:
        ll_sm = neg_ll_sm if data_sm is not None else 0.0
    except NameError:
        ll_sm = 0.0
    try:
        ll_sf = neg_ll_sf if data_sf is not None else 0.0
    except NameError:
        ll_sf = 0.0
    try:
        ll_cou = neg_ll_cou if data_cou is not None else 0.0
    except NameError:
        ll_cou = 0.0

    print(f"[JOINT] LL contributions: SM={-ll_sm:.2f}, SF={-ll_sf:.2f}, COU={-ll_cou:.2f}, Total={ll_total:.2f}")

    # Print gradient norms for each parameter group
    print(f"[JOINT] Gradient norms:")
    print(f"  SM Prefs [0:9]:      {np.linalg.norm(grad[idx_pref_sm[0]:idx_pref_sm[1]]):.6f}")
    print(f"  SF Prefs [9:18]:     {np.linalg.norm(grad[idx_pref_sf[0]:idx_pref_sf[1]]):.6f}")
    print(f"  COU Prefs [18:34]:   {np.linalg.norm(grad[idx_pref_cou[0]:idx_pref_cou[1]]):.6f}")
    print(f"  HOPP_M [34:41]:      {np.linalg.norm(grad[idx_hopp_m[0]:idx_hopp_m[1]]):.6f}")
    print(f"  HOPP_F [41:48]:      {np.linalg.norm(grad[idx_hopp_f[0]:idx_hopp_f[1]]):.6f}")

    if wage_spec == "vw":
        print(f"  WOPP_M [48:54]:      {np.linalg.norm(grad[idx_wopp_m[0]:idx_wopp_m[1]]):.6f}")
        print(f"  WOPP_F [54:60]:      {np.linalg.norm(grad[idx_wopp_f[0]:idx_wopp_f[1]]):.6f}")

    print(f"  Total gradient norm: {np.linalg.norm(grad):.6f}")
    print("-" * 80)
    # ========================================================================
...
def main() -> None:
    ...
    if args.joint:
        ...
        if args.optimizer == "L-BFGS-B":
            ...
            result = minimize(
                ...
                jac=True,
                bounds=bounds,
                options={"disp": True, "maxiter": args.maxiter, "ftol": 1e-12, "gtol": 1e-6},
            )
        else:
    ...
    if args.group == 1:
        ...
        required_cols = ["hours_male", "hours_female", "idhh", "draw"]
