# FR P2a Region-Live — Phase-5 Inference Design — v4

**Mission:** JMP-M05 Stage D, remediation cycle 2 of 2, deputy programme director front-matter admission pass — household-clustered inference design
**Mode:** design only; no implementation, no computation, no commit
**Date:** 2026-07-31
**Target repository path:** `MNL/docs/France_case/P2a/FR_P2a_region_live_phase5_inference_design_v4.md`
**Supersedes:** `FR_P2a_region_live_phase5_inference_design_v3.md`, itself superseding v2, itself superseding v1. **All three are retained unedited** as the objects the Stage-C review and recheck cite. None is amended in place.
**Remediation authority:** `JMP_M05_stageD_cycle1_instruction_v1.md` (rulings R-11–R-12) for the v1 → v2 fixes F1–F7; Goal 1 Manager card **M05-AC-8, ruling R-14** for the v2 → v3 correction of recheck residual 1, implementing manager-selected route (a). Source of record for cycle 2: `FR_P2a_region_live_phase5_inference_methods_recheck_v1.md` §4.1. Deputy programme director admission for the v3 → v4 front-matter identity update and the two accompanying §1.1 corrections (below).
**Commit status:** UNCOMMITTED pending the T-7 micro-recheck, Stage-E manager acceptance, and the deputy programme director's front-matter admission recorded in §1.1
...
#### Revision register — v2 → v3 (Stage-D cycle 2 of 2)

Cycle 2 addresses **recheck residual 1 only** [recheck §4, §5], under Goal 1 Manager ruling R-14 (card M05-AC-8), implementing the manager-selected **route (a)**. The recheck returned `RECHECK FAIL — RESIDUALS` with fixes F1–F4 and F6–F7 and consequential edits C-a–C-e all passing; F5 passed on T-4, T-9, W-4 and the T-19/T-22/W-4 tiers, with the T-7 backward-error coefficient the sole residual.

| Item | Requirement (recheck §4.1, ruling R-14) | Sections changed in v3 |
| --- | --- | --- |
| **F5-R1** | Redefine the coefficient as `κ_BE = K·γ_G = K·G·u/(1−G·u)` so the displayed derivation reproduces the exact value `6.042388811523458e-12`, replacing the leading-order `K·G·u = 6.0423888115224145e-12`; certify an explicitly upward-rounded gate constant `κ_BE_certified = 6.0424e-12 ≥ κ_BE`; recompute and restate the tightness factor against the `1e-10` rank convention | §15 (T-7 row); §16.2 (Meat PSD (T-7) tolerance row) |

**Assertion, per ruling R-14.** No substantive content changed anywhere outside the two locations named in the F5-R1 row. The gate tier, the model, the estimand, the baseline, every other tolerance, and every other section are byte-identical to v2. The one further difference is the file's own version identity in the front matter — filename, version number, supersession line, remediation authority and commit status — which is entailed by R-14's own instruction to produce a new v3 file and is recorded here rather than made silently; it carries no design content. This mirrors row **C-d** of the v1 → v2 block, which the Stage-C recheck passed on the ground that the front-matter changes only record version and review state [recheck §3].

**Nothing in cycle 2 changes any recommended baseline or reopens any matter the recheck passed.** T-7 is minimally loosened only to implement the valid upward-rounded certification, while remaining 16.55× tighter than the rank convention. The correction moves the certified constant from `6.0423888115224145e-12` to `6.0424e-12`, a relative change of `1.8516646205e-6`, in the direction of a marginally more permissive but correctly derived floor; T-7 remains gating.

#### Revision register — v3 → v4 (deputy programme director admission)

Cycle-2-plus addresses two residual defects in v3's own self-description of the F5-R1 change, plus the version identity, all admitted by the deputy programme director rather than remanded to the Goal 1 Manager or the Stage-C reviewer, because none of the three alters design content.

| Item | Requirement | Sections changed in v4 |
| --- | --- | --- |
| **D-PD1** | Correct the relative-change arithmetic in the v2 → v3 annotation: `1.8e-5` was miscomputed; the exact value is `1.8516646205e-6` | §1.1 (v2 → v3 annotation) |
| **D-PD2** | Replace the internally inconsistent claim that cycle 2 "weakens [no] gate" — inconsistent because the same sentence goes on to describe the certified constant as moving to "a marginally more permissive... floor" — with an accurate statement of T-7's minimal, justified loosening | §1.1 (v2 → v3 annotation) |
| **D-PD3** | Front-matter version identity — title, mission line, target repository path, supersession line, remediation authority, and commit status (six lines) | Front matter |

**Assertion.** No substantive content changed anywhere outside the three locations named above. The gate tier, the model, the estimand, the baseline, every other tolerance value, every artifact hash, and every other section are byte-identical to v3. D-PD1 and D-PD2 correct an arithmetic error and an internal inconsistency in v3's own self-description of the v2 → v3 change; neither alters what changed between v2 and v3, only how accurately v3 described it.
...
| **T-7** | Meat validity: `max\|M − Mᵀ\| ≤ 1e-12 · max\|M\|` before symmetrisation; **`min_eig(M) ≥ −κ_BE_certified · max_eig(M)` with `κ_BE_certified = 6.0424e-12`**, the **upward-rounded** certification of the exact backward-error coefficient `κ_BE = K·γ_G = K·G·u/(1−G·u) = 6.042388811523458e-12` (§16.2); rank recorded | gating |
...
| --- | --- | --- |
| Meat PSD (T-7) | `min_eig ≥ −κ_BE_certified × max_eig`, **certified gate constant `κ_BE_certified = 6.0424e-12`**, rounded **upward** from the exact coefficient `κ_BE = K·γ_G = K·G·u/(1−G·u) = 6.042388811523458e-12` | **Quantitatively justified backward-error bound.** `M = S_Iᵀ S_I` is a Gram matrix formed by inner products of length `G = 1,555` in float64 with unit roundoff `u = 2⁻⁵³ = 1.1102230246251565e-16`; the standard floating-point Gram bound gives `γ_G = G·u/(1−G·u) = 1.7263968032924165e-13`, and the dimension factor `K = 35` from the Weyl perturbation of the smallest eigenvalue gives `κ_BE = K·γ_G` exactly — **not** the leading-order approximation `K·G·u`, which omits the `1/(1−G·u)` factor. The certified constant is rounded **upward** (`κ_BE_certified ≥ κ_BE`, margin `1.12e-17`), so the gate is never tighter than the derivation supports. This is **16.55× tighter** than the `1e-10` rank convention [arith] |
