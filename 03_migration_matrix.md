...
## Deferred provenance task (logged, not lost)

**Certified-snapshot reconciliation (from Wave 1.2):** the certified production negLL 238362.79 is not reproducible from any currently-loadable parquet (~140-nat offset vs loadable stems). Root cause is data-side: the spec-documented production input `fr_p3a_bpool_d1w1` lacks its `__mnlmeta.json` engine-ready sidecar, so it can't be loaded via the harness. This is **not a lift defect** (proven: source builders give identical figures on the same on-disk data) and is **separable from the core spine**. It belongs to JMP/app-layer data provenance, not the core package. Address when convenient by locating/regenerating the certified d1w1-derived engine-ready build + sidecar; does not block Waves 1.3–3.
